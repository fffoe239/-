...
ADMIN_CODES = {item.strip() for item in os.environ.get("ADMIN_CODES", "HH35-ADMIN-OWNER-2026,HH35-ADMIN-FRIEND-2026").split(",") if item.strip()}
...
def init_db():
    with db() as connection:
        connection.execute("""CREATE TABLE IF NOT EXISTS codes (id INTEGER PRIMARY KEY AUTOINCREMENT, code TEXT UNIQUE NOT NULL, duration TEXT NOT NULL, created_at TEXT NOT NULL, expires_at TEXT, device_hash TEXT, disabled INTEGER NOT NULL DEFAULT 0)"""); connection.commit()
...
def check_public_sources(value):
    query = value.strip().lower(); results = []
...
def home():
    if request.method == "POST":
        ...
        with db() as connection: row = connection.execute("SELECT * FROM codes WHERE code = ?", (value,)).fetchone()
        if not row: flash("الكود غير صحيح", "error")
            ...
            with db() as connection:
                connection.execute("UPDATE codes SET device_hash = COALESCE(device_hash, ?), expires_at = COALESCE(expires_at, ?) WHERE id = ?", (device_hash(), expires.isoformat(), row["id"])); connection.commit()
...
def checker():
    ...
    if request.method == "POST":
        ...
        kind = request.form.get("kind", "server")
        if kind == "invite":
            # The visitor types only the invite shortcut, e.g. hh35; no discord.gg/ prefix is needed.
...
@admin_required
def admin():
    with db() as connection: rows = connection.execute("SELECT * FROM codes ORDER BY id DESC").fetchall()
...
def create_code():
    ...
    while True:
        ...
        try:
            with db() as connection: connection.execute("INSERT INTO codes (code, duration, created_at) VALUES (?, ?, ?)", (value, duration, now().isoformat())); connection.commit()
...
def disable_code(code_id):
    with db() as connection: connection.execute("UPDATE codes SET disabled = 1 WHERE id = ?", (code_id,)); connection.commit()
    return redirect(url_for("admin"))

@app.post("/admin/codes/<int:code_id>/delete")
@admin_required
def delete_code(code_id):
    with db() as connection: connection.execute("DELETE FROM codes WHERE id = ?", (code_id,)); connection.commit()
    return redirect(url_for("admin"))
